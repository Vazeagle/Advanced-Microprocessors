...
int main( int argc, const char* argv[] ){ char tmp;
unsigned int gen;
...
data_in = pioa->PDSR; //ΑΝΑΓΝΩΣΗ ΤΙΜΩΝ ΕΙΣΟΔΟΥ if( !(data_in & 0x01) ) //ΔΙΑΚΟΠΤΗΣ ΠΑΤΗΜΕΝΟΣ;
{
...
data_out = pioa->ODSR; //ΑΝΑΓΝΩΣΗ ΤΙΜΩΝ ΕΞΟΔΟΥ pioa->SODR = data_out | 0x02;
